"""
NeuroSWAYML — Intoxication / Ataxia Gait Training
===================================================
Dataset: Human Balance Evaluation Database (HBEDB)
Source:  PhysioNet  https://physionet.org/content/hbedb/1.0.0/
  163 subjects, COP force-platform stabilography, Romberg protocols
  Conditions mapped to impairment level:
...
Optionally also reads generic CSV files (acc_x, acc_y, acc_z, label columns)
for any additional IMU-based intoxication dataset placed in --extra-dir.

Download first:
    python data/downloader.py --domain intox

Usage:
    python training/train_intoxication.py
    python training/train_intoxication.py --hbedb-dir data/physionet/hbedb
    python training/train_intoxication.py --extra-dir data/intoxication
    python training/train_intoxication.py --dry-run
"""
...
from data.loaders.intoxication_loader import IntoxicationLoader
...
DEFAULT_HBEDB  = os.path.join(os.path.dirname(__file__), "..", "data", "physionet", "hbedb")
DEFAULT_EXTRA  = os.path.join(os.path.dirname(__file__), "..", "data", "intoxication")
...
def main():
    parser = argparse.ArgumentParser(
        description="Train intoxication/ataxia gait models (HBEDB dataset)"
    )
    parser.add_argument("--hbedb-dir",  default=DEFAULT_HBEDB,
                        help="Path to local HBEDB directory")
    parser.add_argument("--extra-dir",  default=DEFAULT_EXTRA,
                        help="Optional extra IMU CSV directory")
    ...
    print("=" * 60)
    print("  NeuroSWAYML — Intoxication / Ataxia Gait Training")
    print("  Dataset: HBEDB (PhysioNet)")
    print("=" * 60)

    cfg = load_config()

    # ── 1. Load dataset ───────────────────────────────────────────────
    extra = args.extra_dir if os.path.isdir(args.extra_dir) else None
    print(f"\n[1/5] Loading HBEDB dataset from {args.hbedb_dir} …")
    if extra:
        print(f"       + extra IMU CSV dir: {extra}")

    loader = IntoxicationLoader(args.hbedb_dir, extra)
    X, y   = loader.load()

    if len(X) == 0:
        print("\n[ERROR] No data loaded.")
        print("  → Run:  python data/downloader.py --domain intox")
    ...
    print("\n[2/5] Building intoxication domain model stack …")
    domain = DomainModel("intoxication", cfg, MODELS_ROOT)
    ...
    # ── 6. Calibrate Autoencoder on class-0 (sober) ──────────────────
    print("\n[5/5] Calibrating Autoencoder on SOBER samples …")
    normal = X_tr[y_tr == 0]
    print(f"  {len(normal)} sober samples for AE calibration")
    ...
    y_fall_tr = (y_tr == 2).astype(int)   # class-2 = INTOXICATED → highest fall risk
    ...
    # ── 8. Save everything ────────────────────────────────────────────
    print(f"\n  Saving intoxication models → {domain.models_dir}")
    domain.save_all(gc_metrics, lstm_metrics, ae_metrics, fall_metrics)

    elapsed = time.time() - t_start
    print(f"\n{'='*60}")
    print(f"  Intoxication domain training complete in {elapsed:.1f}s")
